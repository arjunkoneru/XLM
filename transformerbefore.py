...
class PredLayer(nn.Module):
    ...
    def forward(self, x, y, get_scores=False):
        ...
        if self.asm is False:
            ...
            loss = linear_combination(red_loss/n, nll, 0)    
        else:
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        self.layer_norm_before = nn.ModuleList()
        if self.is_decoder:
        ...
        for layer_id in range(self.n_layers):
            ...
            self.layer_norm_before.append(nn.LayerNorm(self.dim, eps=1e-12))
            if self.is_decoder:
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None):
        ...
        for i in range(self.n_layers):
            ...
            # self attention
            tensor = self.layer_norm_before[i](tensor)
            attn = self.attentions[i](tensor, attn_mask, cache=cache)
            attn = F.dropout(attn, p=self.dropout, training=self.training)
            if((i != self.skip_res) or self.is_decoder):
                tensor = tensor + attn
