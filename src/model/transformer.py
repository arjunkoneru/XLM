...
class PredLayer(nn.Module):
    ...
    def forward(self, x, y, get_scores=False,smooth_mt=False):
        """
        ...
        if self.asm is False:
            if smooth_mt is False:
                ...
                red_loss = reduce_loss(-log_preds.sum(dim=-1),'mean')
                nll = F.cross_entropy(log_preds, y, reduction='mean')
                loss = linear_combination(red_loss/n, nll, 0.2)    
        else:
...
class TransformerFFN(nn.Module):
    ...
    def __init__(self, in_dim, dim_hidden, out_dim, dropout, gelu_activation,activation_dropout):
        super().__init__()
        self.dropout = dropout
        self.act_dropout = activation_dropout
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        for layer_id in range(self.n_layers):
            ...
            if ('%i_in' % layer_id) in self.memories:
                ...
                self.ffns.append(TransformerFFN(self.dim, self.hidden_dim, self.dim, dropout=self.dropout, gelu_activation=params.gelu_activation,activation_dropout=params.activation_dropout))
    ...
    def predict(self, tensor, pred_mask, y, get_scores,smooth_mt=False):
        """
        ...
        scores, loss = self.pred_layer(masked_tensor, y, get_scores,smooth_mt)
        return scores, loss
