...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.stats = OrderedDict(
            ...
            [('PT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.pt_steps] +
            [('TRPPL-%s-%s' % (l1, l2), []) for l1, l2 in params.trppl_steps]
        )
...
class EncDecTrainer(Trainer):
    ...
    def __init__(self, encoder, decoder, data, params,encoder_lm=None):

        #if(params.reload_lm == ''):
        self.MODEL_NAMES = ['encoder', 'decoder']
        #else:
            #self.MODEL_NAMES = ['encoder', 'decoder', 'encoder_lm']
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        # loss
        if lang1 != lang2:
            _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False,smooth_mt=False)
        else:
    ...
    def pt_step(self, lang1, lang2, lang3, lambda_coeff):
        """
        ...
        (x1, len1), (x3, len3) = self.get_batch('pt', lang1, lang3)
        langs1 = x1.clone().fill_(lang1_id)
        ...
        self.stats[('PT-%s-%s-%s' % (lang1, lang2, lang3))].append(loss.item())
        loss = lambda_coeff * loss
    ...
    def trppl_step(self, lang1, lang2, lambda_coeff):
        """
        Optimize Translation Perpexlity at the decoder
        """
        assert lambda_coeff >=0
        if lambda_coeff ==0:
            return
        ...
        (x1, len1) = self.get_batch('trppl', lang1)
        langs1 = x1.clone().fill_(lang1_id)
        ...
        assert pred_mask.sum().item() == y.size(0)
        # cuda
        #pred_mask, y = to_cuda(pred_mask, y)
        self.encoder_lm.eval()
        # forward / loss
        tensor = self.encoder_lm('fwd', x=x2, lengths=len2, langs=langs2, causal=True)
        masked_tensor = tensor[pred_mask.unsqueeze(-1).expand_as(tensor)].view(-1,self.encoder_lm.dim)
        scores = self.encoder_lm.pred_layer.proj(masked_tensor).view(-1,self.encoder_lm.n_words)
        loss = F.cross_entropy(scores, y ,reduction = 'mean')
        #_, loss = self.decoder('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=False)
        xe_loss = loss.item()*len(y)
        n_words = y.size(0)
        ppl = np.exp(xe_loss/n_words)
        
        self.stats[('TRPPL-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss
        self.optimize(loss)
        # number of processed sentences / words
        self.n_sentences += params.batch_size
