...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if lang in params.ae_steps or lang in params.bt_src_langs or lang in params.trppl_langs:
...
def load_para_data(params, data):
    ...
    required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps + params.pt_steps)
...
def check_data_params(params):
    ...
    params.pt_steps = [tuple(s.split('-')) for s in params.pt_steps.split(',') if len(s) > 0]
    assert all([len(x) == 3 for x in params.pt_steps])
    assert all([l1 in params.langs and l2 in params.langs and l3 in params.langs for l1, l2, l3 in params.pt_steps])
    assert all([l1 != l3 and l1 != l2 and l2!=l3 for l1, l2, l3 in params.pt_steps])
    assert len(params.pt_steps) == len(set(params.pt_steps))
    assert len(params.pt_steps) == 0 or not params.encoder_only

    # Translation Perplixity steps
    params.trppl_steps = [tuple(s.split('-')) for s in params.trppl_steps.split(',') if len(s) > 0]
    assert all([l1 in params.langs and l2 in params.langs for l1, l2 in params.trppl_steps])
    assert all([l1 != l2 for l1, l2 in params.trppl_steps])
    assert len(params.trppl_steps) == len(set(params.trppl_steps))
    assert len(params.trppl_steps) == 0 or not params.encoder_only
    params.trppl_langs = [l1 for l1,_ in params.trppl_steps]
    
    if(params.reload_lm != ''):
        assert os.path.isfile(params.reload_lm)
