...
model_dir_sp=$ROOT/dumped/multi/en-kn-ml-ta-te/supervised/en-ta_smooth
model_path_sp="$model_dir_sp/best-valid_ta-en_mt_bleu.pth"
exp_name="cascaded/$src_lang-$ref_lang-$tgt_lang.sacrebleu"
model_dir_usp=$ROOT/dumped/multi/en-kn-ml-ta-te/unsupervised/ta-kn/
model_path_usp="$model_dir_usp/best-valid_kn-ta_mt_bleu.pth"
exp_id=1
...
if [[ "$tgt_lang" != "en" ]] && [[ "$tgt_lang" != "de" ]]; then
        if [ "$derom_tgt" == "true" ]; then
                ...
                python $charpy -o $DIR/hypothesis.derom.$tgt_lang -r $DIR/reference.derom.$tgt_lang
        else
