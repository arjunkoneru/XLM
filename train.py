...
def get_parser():
    ...
    parser.add_argument("--activation_dropout", type=float, default=0,
                        help="Dropout in the activation of feedforward")
    ...
    parser.add_argument("--skip_res", type=int, default=99,
                        help="Skip residual connection for this layer")
    ...
    parser.add_argument("--lambda_bt", type=str, default="1",
                        help="BT coefficient")
    parser.add_argument("--lambda_pt", type=str, default="1",
                        help="PT coefficient")
    parser.add_argument("--lambda_trppl", type=str, default="1",
                        help="TR PPL  coefficient")
    ...
    parser.add_argument("--pt_steps", type=str, default="",
                        help="Cross Translation steps")
    parser.add_argument("--trppl_steps", type=str, default="",
                        help="Translation Perplexity Steps")
    


    # reload pretrained embeddings / pretrained model / checkpoint
    parser.add_argument("--reload_lm",type=str,default="",
                        help="Reload Language model for translation perplexity")
...
def main(params):
    ...
    # Reload Language model to evaluate translation perplexity
    if(params.reload_lm != ''):
        reloaded = torch.load(params.reload_lm)
        model_params = AttrDict(reloaded['params'])
        logger.info("Supported languages for translation language model: %s" % ", ".join(model_params.lang2id.keys()))

        # build dictionary / build encoder / build decoder / reload weights
        dico_reloaded = Dictionary(reloaded['dico_id2word'], reloaded['dico_word2id'], reloaded['dico_counts'])
        encoder_lm = TransformerModel(model_params, dico_reloaded, is_encoder=True, with_output=True).cuda().eval()
        enc_reload = reloaded['model' if 'model' in reloaded else 'encoder']
        if all([k.startswith('module.') for k in enc_reload.keys()]):
            enc_reload = {k[len('module.'):]: v for k, v in enc_reload.items()}
        encoder_lm.load_state_dict(enc_reload,strict=False)
        for lang in params.lgs.split('-'):
            lang_id = params.lang2id[lang]
            assert lang_id==model_params.lang2id[lang]
        for param in encoder_lm.children():
    ...
    if params.encoder_only:
        ...
        if (params.reload_lm !=''):
            trainer = EncDecTrainer(encoder, decoder, data, params,encoder_lm)
        else:
    ...
    for _ in range(params.max_epoch):
        ...
        while trainer.n_sentences < trainer.epoch_size:
            ...
            # Pivot Translation steps
            for lang1, lang2, lang3 in shuf_order(params.pt_steps):
                trainer.pt_step(lang1, lang2, lang3, params.lambda_pt)

            # Translation Perplexity Steps
            for lang1, lang2 in shuf_order(params.trppl_steps, params):
                trainer.trppl_step(lang1, lang2, params.lambda_trppl)
